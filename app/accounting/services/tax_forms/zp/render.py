"""Render unsigned Obrazac ZP v1.0 XML from ZpPayload."""
...
from accounting.services.tax_forms.zp.payload import ZP_SCHEMA_VERSION, ZpPayload
...
def _append_metapodaci(root: ET.Element, payload: ZpPayload) -> None:
    ...
    uskladjenost.text = 'ObrazacZP-v1-0'
...
def render_zp_xml(payload: ZpPayload) -> bytes:
    """Render unsigned Obrazac ZP XML from canonical payload."""
    root = ET.Element(
        _local('ObrazacZP'),
        {'verzijaSheme': ZP_SCHEMA_VERSION},
    )
    ...
    for index, row in enumerate(payload.rows, start=1):
        ...
        ET.SubElement(isporuka, _local('I2')).text = '0.00'
        ET.SubElement(isporuka, _local('I3')).text = '0.00'
        ET.SubElement(isporuka, _local('I4')).text = _format_decimal(row.services_value)

    ukupno = ET.SubElement(tijelo, _local('IsporukeUkupno'))
    ET.SubElement(ukupno, _local('I1')).text = _format_decimal(payload.total_goods)
    ET.SubElement(ukupno, _local('I2')).text = '0.00'
    ET.SubElement(ukupno, _local('I3')).text = '0.00'
    ET.SubElement(ukupno, _local('I4')).text = _format_decimal(payload.total_services)
